...
int ppm2file(const PPMFile *ppm, const char *path) {
    #ifdef POOL_DEBUG
    INFO("ppm2file at 0x%lx to ppm file %s", (uint64_t)ppm, path)
    #endif
    FILE *ppm_file = fopen(path, "w");
    if (!ppm_file) {
        fprintf(stderr, "[%s:%d] Cannot open ppmfile %s\n", __FILE__, __LINE__, path);
    ...
    // write to ppm file
    fprintf(ppm_file, "P3\n%d %d\n255\n", (uint32_t)ppm->size, (uint32_t)ppm->size);
    for (uint32_t row = 0; row < ppm->size; row++) {
        for (uint32_t col = 0; col < ppm->size; col++) {
            uint32_t idx = row * ppm->size + col;
            fprintf(ppm_file, "%d %d %d\t", ppm->pixels[idx].r, ppm->pixels[idx].g,
                    ppm->pixels[idx].b);
        }
        fprintf(ppm_file, "\n");
    }
...
int pool2ppm(const Pool *pool, PPMFile *ppm) {
    #ifdef POOL_DEBUG
    INFO("pool2ppm at 0x%lx to ppm 0x%lx", (uint64_t)pool, (uint64_t)ppm)
    #endif
    uint32_t entry_num = pool->size * pool->size;
    ppm->pixels = (RGBPixel *)calloc(entry_num, sizeof(RGBPixel));
    // compute R channel
    uint8_t *red = (uint8_t *)calloc(entry_num, sizeof(uint8_t));
    memset(red, 0, entry_num * sizeof(uint8_t));
    for (uint32_t i = 0, num = pool->small_num; i < num; i++) {
    ...
    for (uint32_t row = 0; row < pool->size; row++) {
        for (uint32_t col = 0; col < pool->size; col++) {
            uint32_t idx = row * pool->size + col;
            ppm->pixels[idx].r = (blue[idx] == 0 && red[idx] > 0) ? 255 : 0;
            ppm->pixels[idx].g = 0;
            ppm->pixels[idx].b = blue[idx];
        }
    }
    free(red);
    free(blue);
